...
키보드 도구막대의 완료(✔) 버튼은 iOS/Android 모두 공통 `accessoryClose` 스타일의 흰색(#ffffff)을 사용합니다. Android의 테마별 색상 재정의를 제거했습니다. 버튼 동작·크기와 Undo/Redo 활성 상태는 유지합니다. 이번 색상 수정의 컴파일·린트·테스트·빌드는 미실행입니다.
...
이번 main 전달에는 완료(✔) 버튼 흰색 고정과 배경 효과의 공통 줄 시작점 정렬, 2026-09-11 재빌드 기록을 포함합니다. 해당 UI 수정은 직전 1.0.7(25) APK/AAB 생성 이후의 변경이며 기존 산출물에는 포함되지 않습니다. 수정 후 컴파일·테스트·실기기 검증은 미실행입니다. 별도 스플래시 및 AAB 파일명 스크립트 변경은 보존·제외합니다.


## 다음 앱 버전 — 1.0.8

앱 버전을 1.0.8, Android versionCode를 26으로 변경했습니다. Settings는 Expo 설정의 버전을 읽어 V1.0.8을 표시합니다. 이번 버전의 빌드·테스트·스토어 업로드는 아직 실행하지 않았으며 기존 1.0.7 산출물과 검증 기록은 그대로 보존합니다.
