...
- 최근 기록된 로컬 APK/AAB는 **1.1.2 (30)**, 소스 `afd29b86c1762297f9555a7cdb5a70ac663c38a9`, 실행 ID `20260922-024340-ebd9287d`입니다. 30은 빌드 시 명시한 override입니다. 고정 빌드 폴더·기존 서명·production AdMob으로 생성했으며 전체 래퍼 시간은 686.11초(약 11분 26초), Gradle은 8분 11초였습니다. [실행 기록](artifacts/apk-runs/20260922-024340-ebd9287d/build-result.json)과 [Android 빌드 이력](docs/ANDROID_BUILD_OPTIMIZATION.md)을 참고하세요. 이번 대화의 보상 모달 접근성 격리와 미사용 export 정리를 포함하며, 새 APK의 TalkBack 실기기 확인은 별도입니다.
...
- Galaxy SM-M336K(Android 16)의 최신 설치 APK에서 Test Ad 재생·닫기, 보상 후 잠금 해제·재시작 유지와 이전 보상의 2시간 초과 후 재잠금을 확인했습니다. 정확한 만료 경계 순간은 측정하지 않았습니다. 화면·성능·로그 및 검증 한계는 [9월 22일 QA 기록](docs/QA_20260910.md#2026-09-22--최신-v112-30-설치-apk-최종-qa)에 정리했습니다. 접근성 수정은 소스에만 반영됐으며 새 APK의 TalkBack 검증이 필요합니다. iOS, 구매·복원, 광고 조기 종료, production 공급률·수익 및 Play 처리·버전 코드 사용 가능 여부는 미검증입니다. 날짜별 단락과 커밋·푸시 미실행 표기는 각 작업 당시의 역사적 기록입니다.
...
최근 추가된 Name·Location·Today 표지판 3종은 Pixel 캔버스 내부 이미지 경로에서 공통 배경 효과 오버레이로 이동했습니다. 따라서 일반/Pixel 모드 모두 사진과 Pixel 배경 뒤에 숨지 않고 미리보기와 전체 화면에서 같은 원본 프레임으로 표시됩니다. 현재 표지판 PNG는 흰색 본문까지 완전 불투명하므로 사진은 프레임 안쪽 흰색 영역을 통과해 보이지 않습니다. 사진을 그 영역에 표시하려면 별도의 투명 마스크 변경이 필요합니다.

정리 과정에서 사진과 일반 효과가 공유하던 동일 셰이더의 중복 별칭, 더 이상 의미가 없는 CircleGrid 판별 API, 읽지 않던 locale·dot-size 매개변수, 내부 전용 상수·타입 재-export와 표지판 이동 후 불필요해진 Pixel 이미지 해석을 제거했습니다. 확인된 미사용 실행 파일은 없어 파일 삭제는 하지 않았습니다.

소스와 diff만 정적으로 검토했습니다. 빌드·린트·테스트·실기기 확인은 실행하지 않았으며, 작은/큰 글자와 모든 언어·배경 조합의 시각 결과는 다음 승인된 빌드에서 확인해야 합니다.
