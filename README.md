...
2026-09-09 후속 소스: 큰 적응형 배너·SDK 초기화 3회 재시도·자동 갱신 뷰 유지·test/production 분리·명시적 웹 진단 모드·Android Activity 몰입형 처리를 추가했습니다. [광고 구현 및 검증 제한](docs/ADVERTISING.md)을 참고하세요. 아직 새 컴파일·실기기 측정·APK/AAB 생성은 실행하지 않았습니다.
...
- 배너도 최초 요청과 3초·6초 재시도를 사용합니다. 최종 실패 안내는 10초 후 숨기되 영역 높이와 실패 상태는 유지하며, 문구 숨김으로 추가 요청하지 않습니다. 일반 웹의 광고 미지원 안내는 계속 표시합니다. 배너 컴포넌트를 새로 마운트하면 새 로드 주기이며, 리워드의 공유 실패 상태와는 별도입니다.
