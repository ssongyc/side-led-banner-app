...
## 후속 소스 변경 — 기존 1.0.6 (24) 산출물에는 미포함

- 배너: 최초 요청·실패 후 3초·다시 실패 후 6초의 3회가 모두 실패하면 안내를 10초 표시합니다. 안내 종료 후 60초 뒤에 동일한 3회 주기를 딱 한 번 추가합니다. 추가 주기도 실패하면 안내를 10초 표시하고 중단합니다. 화면 해제 시 타이머를 취소하며 SDK 초기화·설정 실패와 리워드 정책은 변경하지 않습니다.
...
## 최신 생성 APK·AAB — 1.0.6 (24), 2026-09-09
...
- 네이티브 배너는 최초 요청과 실패 후 3초·6초 재시도를 사용합니다. 3회 실패 안내를 10초 표시한 뒤 60초 더 기다려 동일한 주기를 한 번만 추가합니다. 추가 주기도 실패하면 안내를 10초 표시하고 중단합니다. 일반 웹의 광고 미지원 안내는 계속 표시합니다. 배너를 새로 마운트하면 새 주기이며, 리워드의 공유 실패 상태와 별도입니다. 자세한 범위는 [광고 문서](docs/ADVERTISING.md)를 참조하세요.
