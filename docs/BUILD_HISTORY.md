...
## 2026-09-22 V1.1.3 소스 업데이트

- Expo 앱 버전을 `1.1.3`, Android `versionCode`를 `31`로 변경했습니다.
- Settings는 Expo 설정을 읽으므로 App Version에 `V1.1.3`을 표시합니다.
- iOS buildNumber와 EAS 원격 빌드 번호는 변경하지 않았습니다.
- Android 사진 선택 후 시스템 자르기 화면으로 넘어가기 전에 Background 화면이 잠깐 다시 보이지 않도록 전체 화면 검은 커버에서 선택기를 실행하도록 전환 수명주기를 조정했습니다. iOS 흐름은 유지하며 새 APK 실행 검증은 하지 않았습니다.
- 기존 `1.1.2 (30)` APK/AAB는 과거 산출물입니다. 이번 버전 변경 후 빌드·린트·테스트는 실행하지 않았습니다.
...
- 언어별 기본값 4종(Noto Sans SC/KR/TC, LINE Seed JP)은 로컬에 남기되, 언어별 실사용 확률이 높은 문자 기준으로 필터링했습니다. 간체는 GB18030 Level 2 상당, 한국어는 완성형 한글 전체+상용한자 1800, 일본어는 상용한자+인명용한자, 번체(대만)는 국자표 상용+차상용자 기준으로 했습니다.
- 결과적으로 release APK 기준 73.79MB로 용량을 줄였습니다.

## 2026-09-14 V1.1.1 소스 업데이트와 릴리스 시도

- 앱 버전을 `1.1.1`, Android `versionCode`를 `29`로 올렸고 Settings의 App Version 표시도 `V1.1.1`로 맞췄습니다.
- 사용하지 않는 직접 의존성, 초기화 함수, 템플릿 초기화 스크립트와 대체된 스플래시 이미지를 정리했습니다.
- 보상 광고 안내 문구를 실제 지급 시간인 2시간과 일치시켰습니다.
- 프로덕션 광고 설정으로 시작한 릴리스 실행 `20260914-164757-b630a61e`는 `npm ci` 단계에서 잠긴 `zod` 파일의 `EBUSY` 오류로 중단됐습니다. 네이티브 생성과 컴파일은 시작되지 않았으며 V1.1.1 APK/AAB는 생성되지 않았습니다.
- 이 전달에서는 빌드·린트·테스트를 다시 실행하지 않았습니다.
- 현재 최신 로컬 검증 산출물은 main `a4657ac`에서 만든 V1.1.0(28), 실행 기록 `artifacts/apk-runs/20260914-103443-c2041774`입니다. 프로덕션 광고 정적 검증은 통과했지만 실제 기기 광고 노출과 Google Play 업로드는 수행하지 않았습니다.
